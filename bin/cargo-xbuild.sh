...
if [ -z "${RUST_XBUILD_TARGET}" ]; then
    xarch_list=()
    xarch_list+=(x86_64-unknown-linux-musl)
    xarch_list+=(i586-unknown-linux-musl)
    xarch_list+=(i686-unknown-linux-musl)
    xarch_list+=(aarch64-unknown-linux-musl)
    xarch_list+=(powerpc64le-unknown-linux-musl)
    xarch_list+=(riscv64gc-unknown-linux-musl)
    xarch_list+=(x86_64-unknown-freebsd)
    xarch_list+=(i686-unknown-freebsd)
    xarch_list+=(x86_64-unknown-netbsd)
else
